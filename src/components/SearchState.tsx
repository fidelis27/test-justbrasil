...
export function SearchState({ kind, query, onRetry }: SearchStateProps) {
  ...
  if (kind === 'empty') {
    return <div className="state-panel"><span className="state-mark muted-mark" aria-hidden="true">?</span><div><strong>Nenhum resultado encontrado</strong><p>Não encontramos documentos para “{query}”. Tente uma grafia diferente ou termos mais amplos.</p></div></div>;
  }
